...
> Phase: 3
> Depends on: 06 (seat-opened 이벤트), 08 (알림 큐 + chrome.notifications 발송)
>
> **2026-04-29 갱신**: spec 07 폐기로 알림 발송 책임은 spec 08의 `notify`에 위임. `seat-closed` 알림은 발송하지 않음(노이즈 우려, 결정 U-10-1).
...
- 옵션 페이지에 관심 특강 관리 섹션
- 매처: `seat-opened` 이벤트의 `qustnrSn`이 watchlist에 있으면 spec 08의 `notify` 호출 (`watched-seat-opened` 카테고리, 본문에 잔여석 수치 표기)
- 신청 완료 시 자동 제거 (`UPSERT_SOURCE_LECTURE` 성공 핸들러에 hook)
- 특강 종료 시각 지나면 자동 제거

### 제외
- `seat-closed` 알림 (2026-04-29 결정 U-10-1, 발송 안 함)
- 자리 알림 외 시간/장소 변경 알림 (spec 07 폐기로 기능 자체 제거)
...
2. 폴링 중 누군가 취소 → spec 06이 `seat-opened` 발행 → 매처가 매칭 → spec 08 `notify` 호출 (큐 + OS 알림).
3. 사용자가 알림 클릭 → 상세 페이지 이동 → 신청 → 신청 성공 hook이 자동 watchlist 해제.

### 엣지 케이스
- 자리 났다 곧바로 다시 마감: `seat-opened` 알림만 발송, `seat-closed`는 무시.
...
- 알림 본문: `[자리났음] <제목> — 잔여 N석` (잔여석 수 표시, 2026-04-29 결정 U-07-2).
...
- 읽기: `watchedLectures`, lecture 정보, `seat-opened` 이벤트
- 쓰기: `watchedLectures`
- 호출: spec 08 `notify`, `UPSERT_SOURCE_LECTURE` 성공 핸들러 hook
...
- unit (`lecture-watch.test.js`)
  - `seat-opened` 매칭 → spec 08 `notify` 1회 호출, 본문에 잔여석 수치 포함
  - `seat-closed` 이벤트는 무시 (notify 호출 0회)
  - 신청 완료 hook → 자동 제거
...
- watchlist 100건 cap.
- 알림 폭주 방지: 같은 qustnrSn에 대해 같은 카테고리 알림 중복 시 spec 08의 동일 알림 ID 갱신.
- 권한 추가 없음.

## 12. 미해결 질문

- ✅ **U-10-2 결정 (2026-04-30)**: 자신 일정과 완전 겹치는 특강을 관심 등록할 때 **경고만 표시, 등록 허용**. 파워 유저 자율성 우선. 경고 메시지에 겹치는 일정의 시간·제목을 명시해 사용자 판단을 도움. (실수 충돌 등록 후 캘린더 꼬임 버그 발생 시 차단 정책 재검토)
- `@tbd` 자동 제거 시 사용자에게 읽음 처리되지 않은 큐 항목까지 같이 정리할지(현재는 큐는 별개 정책).

## 13. 관련 링크

- 관련 spec: 06 seat 이벤트, 08, 09
- 폐기: spec 07
