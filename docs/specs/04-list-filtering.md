...
### 제외
- **텍스트 검색 → 별도 spec 11로 분리 (2026-04-30 결정 U-04-2)**. CLAUDE.md "한 브랜치 = 한 spec" 규약 일치.
- 정규식 입력
- 필터를 URL로 공유
...
- ✅ **U-04-2 결정 (2026-04-30)**: 텍스트 검색은 본 spec에서 **제외**, 별도 spec 11(`docs/specs/11-text-search.md`)로 분리. spec 04 머지 후 spec 11 진입.
- `@tbd` T-04: 신청 상태 `cancelled`를 어떻게 안정적으로 판정할지(접수내역 페이지를 거쳐야만 알 수 있음).
