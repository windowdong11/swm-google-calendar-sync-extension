...
- ✅ **U-09-1 결정 (2026-04-30)**: 별표 토글은 **멘토 칩 옆 + 멘토 그룹 헤더(spec 03) 둘 다** 노출. 컨텍스트별 즉시 토글이 가능한 action 우선 UX. 단 구현 단계에서는 YAGNI 적용 검토 권장 — "그룹 헤더 토글만 먼저 머지 → 사용 후 칩 단위 추가" 단계 도입 가능. 최종 모양은 둘 다.
