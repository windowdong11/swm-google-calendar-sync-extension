...
- ✅ **U-02-2 결정 (2026-04-30)**: 카테고리 매핑 편집 UI는 **옵션 페이지**에 둔다. 편집 빈도가 낮아 목록 페이지 인-페이지 UI는 과한 노이즈. 편집 빈도가 늘어나는 신호가 보이면 칩 우클릭 메뉴로 이전 검토.
- 🟡 **U-02-1 부분 해소 (2026-04-29)**: site-current/list.html 분석으로 카테고리 컬럼이 list.do에 노출되지 않음 확인. 상세 페이지(view.do)에서만 추출 가능 가능성. mock 갱신은 별도 raw 캡처 도착 시 (B-1).
- `@tbd` T-02: 와일드카드를 정규식까지 허용할지(보안·UX 우려).
